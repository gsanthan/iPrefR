...
import translate.PreferenceTranslator;
...
public class PreferenceMetaData {
	...
	/**
	 * Preference namesOfVariables used in the SMV model 
	 */
	public String[] namesOfVariables;
	/**
	...
	/**
	 * If using this constructor, all the meta data regarding the preference namesOfVariables, 
	...
	/**
	 * Loads preference namesOfVariables used in the preference specification from the SMV model;
	...
	public PreferenceMetaData(String smvFile) {
		try {
			//TODO change the following code to read namesOfVariables using generic translator, not TCP-net specific
			this.variables = new HashSet<PreferenceVariable>(PreferenceTranslator.getVariablesFromSMVModel(smvFile));
			Set<String> varNames = new HashSet<String>();
			for(PreferenceVariable prefVar : variables) {
				varNames.add(prefVar.getVariableName());				
			}
			this.namesOfVariables = varNames.toArray(new String[0]);
			Arrays.sort(namesOfVariables);
		} catch (IOException e) {
			OutputUtil.println("Error in parsing list of namesOfVariables from SMV file");
	...
	/**
	 * Sets the preference namesOfVariables used to the parameter namesOfVariables; and parses the SMV model file to 
	...
	public PreferenceMetaData(String[] variables, String smvFile) {
		this.namesOfVariables = variables;
		this.smvFile = smvFile;
		this.workingFile = smvFile + "-c.smv";
		this.outputFile = workingFile + "-output.txt";
		updateCounterExampleFile();
	...
	public void setNamesOfVariables(String[] variables) {
		this.namesOfVariables = variables;
	}
